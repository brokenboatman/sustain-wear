...
async function fetchDonations(): Promise<void> {
  ...
  try {
    ...
    // call backend API (was incorrectly pointing at a local file path)
    // correct server route is mounted at /api/staff-donations (see server/index.js)
    ...
    years.sort((a, b) => b - a);
    // convert to SelectItem[] for USelect (cast to avoid TS deep type issues)
